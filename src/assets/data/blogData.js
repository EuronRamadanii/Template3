// import images from all-images/blog-img directory
import img01 from "../all-images/blog-img/blog-1.jpg";
// import img02 from "../all-images/blog-img/blog-2.jpg";
// import img03 from "../all-images/blog-img/blog-3.jpg";
// import img04 from "../all-images/blog-img/blog-1.jpg";

const blogData = [
  {
    id: 1,
    title: "The best way to drive cars",
    author: "Ingenieure Elektrotechnik (m/w/d) - München oder Graz",
    date1: "Aufgaben: ",
    date2: "Qualifikation: ",
    date3: "Wir bieten Ihnen u. a. :",

    
    time: "9pm",
    imgUrl: img01,
    description1:
      "Angepasst an Ihre Vorkenntnisse werden Sie von erfahrenen Kollegen in das Thema Funktionale Sicherheit eingearbeitet",
      description2:
      "Danach beraten Sie unsere Kunden in der Entwicklung und Implemen­tierung von Methoden und Prozessen der Funktionalen Sicherheit",
      description3:
      "Hierbei führen Sie die Systemanalysen unter Berücksichtigung von ASIL- und sicherheitsorientierten Prämissen durch und definieren Anfor­de­rungen an die Systeme oder Teilsysteme",
      description4:
      "Sie entwickeln Sicherheitskonzepte und leiten Anforderungen ab",
      description5:
      "Sie organisieren und führen Reviews für Arbeitsprodukte der Funktionalen Sicherheit durch",
      description11:
      "Abgeschlossenes Ingenieurstudium in der Fachrichtung Elektro­technik, Technische Informatik, Sicherheitsingenieurwesen o. ä.",
      description22:
      "Interesse an der Absicherung von funktions- und sicherheitskritischen Komponenten",
      description33:
      "Verständnis von Normen und Standards sowie deren Einhaltung, wie z. B. ISO 26262, IEC 61508, ISO 27001 oder BSI 100 1-3",
      description44:
      "Reisebereitschaft",
      description55:
      "Fließende Deutsch- und sichere Englischkenntnisse in Wort und Schrift setzen wir voraus",
      description111:
      "Abwechslungsreiche Projekte",
      description222:
      "Flexible Arbeitszeiten",
      description333:
      "Fließende Deutsch- und sichere Englischkenntnisse in Wort und Schrift setzen wir voraus",
    quote:
      "Wir freuen uns, Sie kennenzulernen. Senden Sie Ihre Bewerbungsunterlagen mit Angabe Ihres möglichen Eintrittstermins sowie Ihrer Gehaltsvorstellung an: personal@context.ag",
  },
  {
    id: 2,
    title: "If your car battery is down",
    author: "Functional Safety Engineer (m/w/d) - Automotive - München, Graz",
    date1: "Aufgaben: ",
    date2: "Qualifikation: ",
    date3: "Wir bieten Ihnen u. a. :",

    
    time: "9pm",
    imgUrl: img01,
    description1:
      "Sie führen Systemanalysen unter Berücksichtigung von ASIL- und sicherheitsorientierten Prämissen durch und definieren Anforderungen an die Systeme oder Teilsysteme",
      description2:
      "Sie entwickeln Sicherheitssysteme, erstellen und analysieren Sicherheitskonzepte",
      description3:
      "Sie führen Risikoanalysen (HARA, FMEA) durch, untersuchen unterschiedliche Fehlerklassen und deren Auswirkung (FMEDA) sowie Abhängigkeiten (FTA, DFA) im System",
      description4:
      "Sie spezifizieren Anforderungen und prüfen die Verifikation dieser",
      description5:
      "Sie erstellen Validierungskonzepte und begleiten die Validierungen",
      description6:
      "Sie organisieren und führen Reviews für Arbeitsprodukte der Funktionalen Sicherheit durch",
      description11:
      "Abgeschlossenes Ingenieurstudium in der Fachrichtung Elektrotechnik, Technische Informatik, Mathematik, Sicherheitsingenieurswesen o. ä.",
      description22:
      "Einschlägige Berufserfahrung im Bereich System- oder Komponentenentwicklung (wünschenswert alternative Antriebe, (Teil-)autonome Fahrfunktionen)",
      description33:
      "Gute Kenntnisse im Bereich Mess- und Prüftechnik",
      description44:
      "Kenntnisse der Normen und Standards, wie z. B. A-SPICE, ISO 26262, IEC 61508, ISO 27001 oder ISO 16750 von Vorteil",
      description55:
      "Strukturierte und präzise Arbeitsweise sowie hohe Lernbereitschaft",
      description66:
      "Reisebereitschaft und Fließende Deutsch- und sichere Englischkenntnisse setzen wir voraus",
      description111:
      "Projekte in einem innovativen Umfeld",
      description222:
      "Flache Hierarchien und flexible Arbeitszeiten",
      description333:
      "Weitgefächertes Aufgabengebiet und Ein außergewöhnlich gutes Arbeitsklima",

    quote:
      "Wir freuen uns, Sie kennenzulernen. Senden Sie Ihre Bewerbungsunterlagen mit Angabe Ihres möglichen Eintrittstermins sowie Ihrer Gehaltsvorstellung an: personal@context.ag",
  },
  {
    id: 3,
    title: "If your car battery is dow",
    author: "Functional Safety Manager (m/w/d) - Automotive - München, Graz",
    date1: "Aufgaben: ",
    date2: "Qualifikation: ",
    date3: "Wir bieten Ihnen u. a. :",

    
    time: "9pm",
    imgUrl: img01,
    description1:
      "Sie führen Systemanalysen unter Berücksichtigung von ASIL- und sicherheitsorientierten Prämissen durch und definieren Anforderungen an die Systeme oder Teilsysteme",
      description2:
      "Sie entwickeln Sicherheitssysteme, erstellen und analysieren Sicherheitskonzepte",
      description3:
      "Sie führen Risikoanalysen (HARA, FMEA) durch, untersuchen unterschiedliche Fehlerklassen und deren Auswirkung (FMEDA) sowie Abhängigkeiten (FTA, DFA) im System",
      description4:
      "Sie spezifizieren Anforderungen und prüfen die Verifikation dieser",
      description5:
      "Sie erstellen Validierungskonzepte und begleiten die Validierungen",
      description6:
      "Sie organisieren und führen Reviews für Arbeitsprodukte der Funktionalen Sicherheit durch",
      description11:
      "Abgeschlossenes Ingenieurstudium in der Fachrichtung Elektrotechnik, Technische Informatik, Mathematik, Sicherheitsingenieurswesen o. ä.",
      description22:
      "Einschlägige Berufserfahrung im Bereich System- oder Komponentenentwicklung (wünschenswert alternative Antriebe, (Teil-)autonome Fahrfunktionen)",
      description33:
      "Gute Kenntnisse im Bereich Mess- und Prüftechnik",
      description44:
      "Kenntnisse der Normen und Standards, wie z. B. A-SPICE, ISO 26262, IEC 61508, ISO 27001 oder ISO 16750 von Vorteil",
      description55:
      "Strukturierte und präzise Arbeitsweise sowie hohe Lernbereitschaft",
      description66:
      "Reisebereitschaft und Fließende Deutsch- und sichere Englischkenntnisse setzen wir voraus",
      description111:
      "Projekte in einem innovativen Umfeld",
      description222:
      "Flache Hierarchien und flexible Arbeitszeiten",
      description333:
      "Weitgefächertes Aufgabengebiet und Ein außergewöhnlich gutes Arbeitsklima",

    quote:
      "Wir freuen uns, Sie kennenzulernen. Senden Sie Ihre Bewerbungsunterlagen mit Angabe Ihres möglichen Eintrittstermins sowie Ihrer Gehaltsvorstellung an: personal@context.ag",
  },
  // {
  //   id: 2,
  //   title: "If your car battery is down",
  //   author: "Functional Safety Manager (m/w/d) - Automotive - München, Graz",
  //   date: "12 Dec, 2020",
  //   time: "9pm",
  //   imgUrl: img02,
  //   description:
  //     " Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.",
  //   quote:
  //     "Voluptua dolore takimata dolor sadipscing nonumy consetetur et. Stet sed dolores dolores dolores dolores eos.",
  // },

  // {
  //   id: 3,
  //   title: "If your car battery is down",
  //   author: "Functional Safety Manager (m/w/d) - Automotive - München, Graz",
  //   date: "12 Dec, 2020",
  //   time: "9pm",
  //   imgUrl: img03,
  //   description:
  //     " Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.",
  //   quote:
  //     "Voluptua dolore takimata dolor sadipscing nonumy consetetur et. Stet sed dolores dolores dolores dolores eos.",
  // },
  // {
  //   id: 4,
  //   title: "The best way to give trip",
  //   author: "System Engineers (m/w/d) - München",
  //   date: "12 Dec, 2020",
  //   time: "9pm",
  //   imgUrl: img01,
  //   description:
  //     " Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.",
  //   quote:
  //     "Voluptua dolore takimata dolor sadipscing nonumy consetetur et. Stet sed dolores dolores dolores dolores eos.",
  // },
  {
    id: 4,
    title: "The best way to give trip",
    author: "System Engineers (m/w/d) - München",
    date1: "Ihre Aufgaben: ",
    date2: "Ihr Profil: ",
    date3: "Wir bieten Ihnen u. a. :",

    
    time: "9pm",
    imgUrl: img01,
    description1:
      "Systeme, die Sie absichern, kommen aus den Bereichen autonomes Fahren, antriebs- oder fahrdynamische Regelsysteme und alternative Antriebe",
      description2:
      "Für diese Systeme erbringen Sie Nachweise, dass die neu entwickelten Funktionen die Anforderungen der funktionalen Sicherheit erfüllen",
      description3:
      "Innerhalb der Projektsteuerung erfahren Sie die Anwendung von agilen Entwicklungs­methoden in Verbindung mit „traditionellen“ Vorgehensmodellen",
      description4:
      "Im zugehörigen Prozessmanagement kommen bekannte Referenzmodelle aus den Bereichen Automotive SPICE und CMMI zum Einsatz",
      // description5:
      // "Sie erstellen Validierungskonzepte und begleiten die Validierungen",
      // description6:
      // "Sie organisieren und führen Reviews für Arbeitsprodukte der Funktionalen Sicherheit durch",
      description11:
      "Abgeschlossenes, technisches Studium (Elektrotechnik, Informatik, Embedded Systems, Systems Engineering oder vergleichbare Studienrichtung)",
      description22:
      "Mind. 3 Jahre Berufserfahrung in der Elektronikentwicklung sind wünschenswert, wir arbeiten aber auch gerne Absolventen o. g. Studienrichtungen ein",
      description33:
      "Gute Kenntnisse in Fahrzeugfunktionen, insbesondere die in Hard- und Softwarefunktionen umgesetzt werden (ECUs, Low-/High-Level Software, Buskommunikationen)",
      description44:
      "Kenntnisse in modellbasierter Entwicklung (MBSE) wünschenswert",
      description55:
      "Fließende Deutsch- und sichere Englischkenntnisse setzen wir voraus",
      // description66:
      // "Reisebereitschaft und Fließende Deutsch- und sichere Englischkenntnisse setzen wir voraus",
      description111:
      "Weitgefächertes und abwechslungsreiches Aufgabengebiet in o. g. Themengebieten",
      description222:
      "Flexible Arbeitszeiten und ein außergewöhnlich gutes Arbeitsklima",
      description333:
      "Sehr gute Anbindung an den öffentlichen Nahverkehr",
    quote:
      "Wir freuen uns, Sie kennenzulernen. Senden Sie Ihre Bewerbungsunterlagen mit Angabe Ihres möglichen Eintrittstermins sowie Ihrer Gehaltsvorstellung an: personal@context.ag",
  },

  
  
  // {
  //   id: 5,
  //   title: "System Engineers (m/w/d) - München",
  //   author: "Systemingenieure (m/w/d) – Schwerpunkt technische Analysen - Graz (Österreich)",
  //   date: "12 Dec, 2020",
  //   time: "9pm",
  //   imgUrl: img01,
  //   description:
  //     " Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.  Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam. Dolor labore lorem no accusam sit justo sadipscing labore invidunt voluptua, amet duo et gubergren vero gubergren dolor. At diam.",
  //   quote:
  //     "Voluptua dolore takimata dolor sadipscing nonumy consetetur et. Stet sed dolores dolores dolores dolores eos.",
  // },

  {
    id: 5,
    title: "The best way to give",
    author: "Systemingenieure (m/w/d) – Schwerpunkt technische Analysen - Graz (Österreich)",
    date1: "Aufgaben: ",
    date2: "Qualifikation: ",
    date3: "Wir bieten Ihnen u. a. :",

    
    time: "9pm",
    imgUrl: img01,
    description1:
      "Sie unterstützen unsere Kunden vor Ort sowohl präventiv als auch reaktiv bei der systematischen Analyse ihrer Entwicklungsumfänge vom Gesamtsystem bis Komponentendesign (Mechanik, Elektrik, Hardware, Software)",
      description2:
      "Sie begleiten unsere Kunden bei der Entwicklung von Maßnahmen auf Basis der durchgeführten Analysen (z. B. FMEA, FTA, HARA, FMEDA)",
      description3:
      "Sie gestalten und bedienen die Schnittstellen Ihrer Analyse im Hinblick auf Konzeptentwicklung, Anforderungsmanagement, Verifikation und Validierung",
      description4:
      "Nach einer umfangreichen Einarbeitung sind Sie verantwortlich für die Planung, Erstellung und Durchführung von Beratungs- und Dienstleistungsprojekten im Bereich technischer Analysen.",
      // description5:
      // "Sie erstellen Validierungskonzepte und begleiten die Validierungen",
      // description6:
      // "Sie organisieren und führen Reviews für Arbeitsprodukte der Funktionalen Sicherheit durch",
      description11:
      "Interesse an Methoden in Zusammenhang mit Sicherheit und Qualität in der automotiven Produktentwicklung (ISO 26262, IATF16949)",
      description22:
      "Erfolgreich abgeschlossene technische Ausbildung (TU, FH, HTL)",
      description33:
      "Idealerweise Projekterfahrung in der Automobilbranche (vorzugsweise Entwicklungs- oder Qualitätsbereich)",
      description44:
      "Ausgeprägtes analytisches Denkvermögen sowie systematische Herangehensweise an Aufgaben und Problemstellungen",
      description55:
      "Interesse an verschiedensten technischen Themenstellungen",
      description66:
      "Sicheres Auftreten, gute Kommunikationsfähigkeit sowie eine selbständige und eigenverantwortliche Arbeitsweise und Fließende Deutsch- und sichere Englischkenntnisse in Wort und Schrift",
      description111:
      "Umfangreiche Einarbeitung durch ein erfahrenes Team",
      description222:
      "Abwechslungsreiche Projekte in verschiedenen technischen Bereichen und Eigenverantwortliches Arbeiten",
      description333:
      "Flexible Arbeitszeiten und Ein außergewöhnlich gutes Arbeitsklima",

    quote:
      "Wir freuen uns, Sie kennenzulernen. Senden Sie Ihre Bewerbungsunterlagen mit Angabe Ihres möglichen Eintrittstermins sowie Ihrer Gehaltsvorstellung an: personal@context.ag",
  },
];

export default blogData;
